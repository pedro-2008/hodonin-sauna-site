...
import { 
  ...
  TapeIcon, 
  WhirlpoolIcon 
} from "@/components/decorative/HandDrawnIcons";
    ...
    <section id="services" ref={ref} className="py-32 px-6 relative overflow-hidden bg-gradient-services">
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className={`text-center mb-20 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <h2 className="font-display text-5xl md:text-6xl lg:text-7xl font-medium mb-6 elegant-underline inline-block" style={{ color: 'hsl(var(--services-heading))' }}>
            Naše služby
          </h2>
          <p className="text-xl max-w-2xl mx-auto mt-10 font-light" style={{ color: 'hsl(var(--services-main-text))' }}>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {services.map((service, index) => {
            ...
            return (
              <Card 
                key={index} 
                className={`service-card group cursor-pointer border-0 ${
              ...
              >
                ...
                <CardContent className="pt-0">
                  <p className="text-sm mb-4 leading-relaxed font-light" style={{ color: 'hsl(var(--services-main-text))' }}>
                    {service.description}
                  </p>
                  <ul className="space-y-2">
                    {service.features.map((feature, featureIndex) => (
                      <li key={featureIndex} className="flex items-center text-sm gap-2" style={{ color: 'hsl(var(--services-accent-text))' }}>
                  ...
                  {/* Hover Arrow */}
                  <div className="mt-5 flex items-center gap-2 text-luxury-gold-dark opacity-0 group-hover:opacity-100 transform translate-x-[-10px] group-hover:translate-x-0 transition-all duration-300">

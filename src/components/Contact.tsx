...
export const Contact = () => {
  ...
  const contactInfo = [{
    icon: MapPin,
    label: "Adresa",
    value: "Lipová alej 4110/23, Hodonín"
  }, {
    icon: Phone,
    label: "Telefon",
    value: "+420 737 443 674"
  }, {
    icon: Mail,
    label: "Email",
  ...
  return <section id="contact" ref={ref} className="py-32 px-6 relative overflow-hidden bg-gradient-contact">
      ...
      <div className="max-w-6xl mx-auto relative">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          backgroundColor: 'hsl(var(--contact-card-bg))'
            ...
            <CardHeader className="pb-4">
              ...
              }}>Otevírací doba - květen 2026</CardTitle>
              </div>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                {schedule.map((item, index) => <div key={index} className={`grid grid-cols-[100px_1fr_80px] items-center gap-4 p-3 rounded-xl transition-all duration-300 ${item.time === "Zavřeno" ? "bg-red-500/5" : "bg-muted/30 hover:bg-muted/50"}`}>
          ...
          transitionDelay: '150ms'
            ...
            <CardContent className="space-y-4">
              {contactInfo.map((item, index) => <div key={index} className="flex items-start gap-4 p-4 rounded-xl bg-muted/30 hover:bg-muted/50 transition-all duration-300 group">
                  ...
                  <div>
                    <p className="text-sm mb-1 font-light" style={{
                  color: 'hsl(var(--contact-card-secondary))'
                }}>{item.label}</p>
        ...
        {/* Map Card */}
        <div className={`mt-8 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{
        transitionDelay: '300ms'
      }}>
          <Card className="luxury-card overflow-hidden border-0" style={{
          backgroundColor: 'hsl(var(--contact-card-bg))'
        }}>
            <CardHeader>
              <div className="flex items-center gap-4">
